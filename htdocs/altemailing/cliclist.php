...
/**
 *   	\file       htdocs/altemailing/cliclist.php
 *		\ingroup    altemailing
 *		\brief      Thirdparties clic List page for Campagne
 */
...
require_once __DIR__.'/class/campagne.class.php';

// Load translation files required by the page
$langs->loadLangs(array("other", "altemailing@altemailing" ));
...
if($mode == "open"){
	$object = new CampagneRead($db);
}else{
	$object = new CampagneClics($db);
}
$extrafields = new ExtraFields($db);
$hookmanager->initHooks(array('campagnecliclist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->altemailing->campagne->creer;
	//$permissiontodelete = $user->rights->altemailing->campagne->supprimer;
...
if (empty($conf->altemailing->enabled)) accessforbidden('Module Alternative Emailing not enabled');
...
$help_url = '';
if($mode == "open"){
	$title = $langs->trans("ReadsList");
}else{
	$title = $langs->trans("CampagneClicsList");
}
$morejs = array();
$morecss = array();
...
$head = campagnePrepareHead($campagne);
$ctx = 'clic';
if($mode == "open"){
	$ctx = "read";
}
print dol_get_fiche_head($head, $ctx, $langs->trans("Campagne"), -1, $campagne->picto);
...
$morehtmlref .= $langs->trans("Campagne")." ".$campagne->id;
$morehtmlref .= '</div>';
$morehtmlref .= '<div class="refidno">';
$morehtmlref .= $langs->trans("Du")." ".dol_print_date($campagne->datdeb,'%d %B %Y', 'tzuser', $langs)."<br>";
$morehtmlref .= $langs->trans("au")." ".dol_print_date($campagne->datfin,'%d %B %Y', 'tzuser', $langs);
$morehtmlref .= '</div>';


dol_banner_tab($campagne, 'ref', $linkback, 1, 'ref', 'ref', $morehtmlref);
...
$newcardbutton = '';
//$newcardbutton .= dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-bars imgforviewmode', $_SERVER["PHP_SELF"].'?id='.$id.'&mode=common'.preg_replace('/(&|\?)*mode=[^&]+/', '', $param), '', ((empty($mode) || $mode == 'common') ? 2 : 1), array('morecss'=>'reposition'));
