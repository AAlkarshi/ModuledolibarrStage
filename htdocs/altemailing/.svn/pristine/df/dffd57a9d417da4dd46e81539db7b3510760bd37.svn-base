...
/**
 *   	\file       htdocs/altemailing/list.php
 *		\ingroup    altemailing
 *		\brief      List page for Campagne
 */
...
$hookmanager->initHooks(array('campagnelist')); // Note that conf->hooks_modules contains array
...
if (empty($conf->altemailing->enabled)) accessforbidden('Module not enabled');
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Campagne';
	$objectlabel = 'Campagne';
	$uploaddir = $conf->altemailing->dir_output;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/altemailing/card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/altemailing/card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
