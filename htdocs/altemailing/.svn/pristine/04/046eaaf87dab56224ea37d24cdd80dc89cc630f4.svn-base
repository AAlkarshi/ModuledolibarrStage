 ...
 */
class modaltemailing extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->numero = 520044; // TODO Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve an id number for your module

		$this->rights_class = 'altemailing';
		...
		$this->description = "Alternative Emailing";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "Envoyer un mail aux tiers liés à une catégorie";

		// Author
		$this->editor_name = 'Alternative ERP';
		...
		*/
		...
		$this->rights[$r][1] = 'LireCampagne';
		$this->rights[$r][4] = 'campagne';
		$this->rights[$r][5] = 'lire';
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1);
		$this->rights[$r][1] = 'CreerCampagne';
		$this->rights[$r][4] = 'campagne';
		$this->rights[$r][5] = 'creer';
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1);
		$this->rights[$r][1] = 'ModifierCampagne';
		$this->rights[$r][4] = 'campagne';
		$this->rights[$r][5] = 'modifier';
		$r++;
		$this->rights[$r][0] = $this->numero . sprintf("%02d", $r + 1);
		$this->rights[$r][1] = 'SupprimerCampagne';
		$this->rights[$r][4] = 'campagne';
		...
		$this->menu[$r++] = array(
			...
			'type'=>'left',
			'titre'=>'EmailingCampagne',
			'prefix' => img_picto('', $this->picto_at, 'class="infobox-project"').' ',
			'mainmenu'=>'commercial',
			'leftmenu'=>'emailingcampagne',
			'url'=>'',
			'langs'=>'altemailing@altemailing',
			'position'=>1000 + $r,
			'enabled'=>'isModEnabled("altemailing")',
			'perms'=>'$user->hasRight("altemailing", "campagne", "lire")', 
			'target'=>'',
			'user'=>0, // 
		);
		$this->menu[$r++] = array(
			'fk_menu'=>'fk_mainmenu=commercial,fk_leftmenu=emailingcampagne',
			'type'=>'left',
			'titre'=>'CreerEmailingCampagne',
			'prefix' => '',
			'mainmenu'=>'commercial',
			'leftmenu'=>'commercial_emailingcampagne_creer',
			'url'=>'/altemailing/card.php?action=create',
			'langs'=>'altemailing@altemailing',
			'position'=>1000 + $r,
			'enabled'=>'isModEnabled("altemailing")',
			'perms'=>'$user->hasRight("altemailing", "campagne", "creer")', 
			'target'=>'',
			'user'=>0, // 
		);
		$this->menu[$r++] = array(
			'fk_menu'=>'fk_mainmenu=commercial,fk_leftmenu=emailingcampagne',
			'type'=>'left',
			'titre'=>'ListEmailingCampagne',
			'prefix' => '',
			'mainmenu'=>'commercial',
			'leftmenu'=>'commercial_emailingcampagne_list',
			'url'=>'/altemailing/list.php',
			'langs'=>'altemailing@altemailing',
			'position'=>1000 + $r,
			'enabled'=>'isModEnabled("altemailing")',
			'perms'=>'$user->hasRight("altemailing", "campagne", "lire")', 
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/altemailing/sql/');
		if ($result < 0) {
