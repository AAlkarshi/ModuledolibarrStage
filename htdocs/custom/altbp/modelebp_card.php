...
//AJOUT DE MA PART   CHEMIN AFIN DE RATTACHER modelebp.class
dol_include_once('/altbp/class/modelebp.class.php');


dol_include_once('/mymodule/lib/mymodule_myobject.lib.php');

// Load translation files required by the page
$langs->loadLangs(array("altbp@altbp", "other"));
...
$object = new modelebp($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->altbp->dir_output.'/temp/massgeneration/'.$user->id;
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('altbp', 'modelebp', 'lire');
	$permissiontoadd = $user->hasRight('altbp', 'modelebp', 'creer'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('altbp', 'modelebp', 'supprimer') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('altbp', 'modelebp', 'creer'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('altbp', 'modelebp', 'creer'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->altbp->multidir_output[isset($object->entity) ? $object->entity : 1].'/modelebp';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/altbp/modelebp_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/altbp/modelebp_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
//MODIFICATION TITRE
$title = $langs->trans("Bulletin de Paie")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("modèle"));
}
$help_url = '';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-modelebp page-card');
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Bulletin de Paie"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("modelebp"), -1, $object->picto, 0, '', '', 0, '', 1);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/altbp/modelebp_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
...
// BOUTON SOUS LA BOX
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('altbp:modelebp', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('modelebp'));
