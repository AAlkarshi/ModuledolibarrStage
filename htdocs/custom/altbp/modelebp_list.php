...
//AJOUT EFFECTUE
require_once __DIR__.'/class/modelebp.class.php';
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('altbp', 'modelebp', 'read');
	$permissiontoadd = $user->hasRight('altbp', 'modelebp', 'write');
	$permissiontodelete = $user->hasRight('altbp', 'modelebp', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("altbp")) {
	accessforbidden('Module altbp not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Bulletin de Paie';
	$objectlabel = 'Bulletin de Paie';
	$uploaddir = $conf->altbp->dir_output;
...
$title = $langs->trans("Bulletin de Paie");
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/altbp/modelebp_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/altbp/modelebp_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
